...
func TestAddGetDelete(t *testing.T) {
	...
	addedParcel, err := store.Get(parcel.Number)
	require.NoError(t, err)
	...
	require.ErrorIs(t, err, sql.ErrNoRows) // проверяем, что запрос выдаёт ошибку отсутствия строки после удаления
}
...
func TestSetStatus(t *testing.T) {
	...
	addedParcel, err := store.Get(parcel.Number)
	require.NoError(t, err)
	require.Equal(t, ParcelStatusDelivered, addedParcel.Status)
}
